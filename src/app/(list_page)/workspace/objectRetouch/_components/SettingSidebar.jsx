/* 
  ...
  modified settings / advanced settings
*/ 
          ...
          return (
            <div className="flex gap-2">
              <input
                ...
                defaultValue={params[param.id]}
                onChange={(e) => handleParamChange(param.id, e.target.defaultValue)}
              />
              <button
  ...
  return (
    <>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-medium mb-1 text-gray-300">Inpainting Prompt</label>
            <textarea
              value={params.prompt}
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <label className="block text-xs font-medium text-gray-300">Diffusion Steps</label>
            {renderParameter(parameterDefinitions.find(p => p.id === 'diffusionSteps'))}
          </div>
          <div>
            <label className="block text-xs font-medium mb-1 text-gray-300">Positive Prompt</label>
            <textarea
              value={params.positive_prompt || ''}
              onChange={(e) => handleParamChange('positive_prompt', e.target.value)}
              className="w-full p-3 text-sm bg-gray-800/20 rounded-md focus:ring-0 focus:outline-none resize-y min-h-[30px] border border-gray-700"
              rows={2}
            />
          </div>
          <div>
