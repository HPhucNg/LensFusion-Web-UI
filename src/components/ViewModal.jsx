    ...
    return (
        <div className='md:w-[80%] md:flex-row flex flex-col font-sans relative rounded-xl shadow-md overflow-hidden' style={{ background: 'var(--modal-background)' }}>
            ...
            <div className='flex flex-1'> {/* right */}
                ...
                {showCommentsOverlay && (
                    <div className="absolute top-0 right-0 w-full md:w-[50%] h-full bg-[var(--card-background)] backdrop-blur-lg z-50 rounded-xl shadow-lg p-4 overflow-y-auto transition-all duration-300">
                        {/* close button */}
                        <button onClick={handleCloseComments} className="absolute top-2 right-2 transform hover:scale-90 cursor-pointer rounded-lg bg-[var(--border-gray)] backdrop-blur-sm border">
                            <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                            </svg>
                        </button>

                        <div className="pt-8 w-[90%]"> {/* give room for the close button */}
                            <p className='font-semibold text-md p-2 '>{comments.length} Comments</p>
                            <div className="comments-container max-h-[200px] border-2 p-2 ml-2 w-full rounded-md overflow-y-auto ">
                            ...
                            {visibleComments < comments.length && (
                                <button onClick={handleLoadMore} className="text-blue-400 pl-2 text-xs">
                                    Load More
                                </button>
                            )}

                            {/* add comment */}
                            <form className='pt-6 pb-6 w-full' onSubmit={handleSubmit}>
